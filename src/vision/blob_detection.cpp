...
std::vector<Blob> blob_detection(const cv::Mat& img)
{
	int color = 0;
	int channels = 2;
	...
	for (int i = 0; i < num_blobs; i++)
	{
		...
		{
			result.push_back(Blob(color, channels, blobsPoints[i], perimetersPoints[i], img));
		}
	}

	// sort the blobs from greatest area to least area
	std::sort(result.begin(), result.end(), 
		[](const Blob a, const Blob b)->bool
		{
